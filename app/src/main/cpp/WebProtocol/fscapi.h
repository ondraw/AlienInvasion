...
/** @cond DEV */
	...
	#define MINKSYNCML_EXPORT
	#define MINKCORE_EXPORT
	...
	/////////////////////////////////////////////////////
	#ifndef XMLCONFIG
		#define XMLCONFIG //XML Config를 구현할려고 할때.
	#endif

	#if defined(_MINKSYNCML_EXPORT_)
	#define MINKSYNCML_EXPORT __declspec(dllexport)	
	#else
	#define MINKSYNCML_EXPORT __declspec(dllimport)
	#endif



	#if defined(_MINKCORE_EXPORT_)
	#define MINKCORE_EXPORT __declspec(dllexport)	
	#else
		#if defined(_MINKMODULE_EXPORT_) //Core와 모듈이 소스를 같이 쓰기 때문에 이렇게 분리를 해야한다.
			#define MINKCORE_EXPORT
		#else 
			#define MINKCORE_EXPORT __declspec(dllimport)
		#endif
	
	#endif

	
	#if defined(MINKBASE_EXPORTS)
		#define MINKBASE_EXPORT_F __declspec(dllexport)	
	#else
		#define MINKBASE_EXPORT_F __declspec(dllimport)
	#endif
	/////////////////////////////////////////////////////
	


	#if defined(_MINKMODULE_EXPORT_)
		#define MINKMODULE_EXPORT __declspec(dllexport)
	#else
		#define MINKMODULE_EXPORT __declspec(dllimport)
	#endif
	...
	////////////////////////////////////////////////////
	  ...
	  //Added By Song
	  MINKBASE_EXPORT_F void *operator new(size_t s, char* file, int line);
	  MINKBASE_EXPORT_F void *operator new[](size_t s, char* file, int line);  
	  MINKBASE_EXPORT_F void deleteRelease(void* p);
	  MINKBASE_EXPORT_F void printMemLeaks();

	  //CObject는 확장된 new 연산자를 실행하지만, delete 연산자는 실행이 안된다.
      inline void* __cdecl operator new(size_t s)        { return ::operator new(s, __FILE__, __LINE__); }
	  inline void* __cdecl operator new[](size_t s)		 { return ::operator new(s, __FILE__, __LINE__); }

	  inline void __cdecl operator delete(void* p)		 { deleteRelease(p); }
	  inline void __cdecl operator delete[] (void* p)	 { deleteRelease(p); }
