 ...
 *
 */
MINKBASE_EXPORT_F void safeDelete(char*  p[]);

MINKBASE_EXPORT_F void safeDel(char** p);
 ...
 */
MINKBASE_EXPORT_F void timestampToAnchor(unsigned long timestamp, char anchor[21]);

/**
 * inverse operation for timestampToAnchor(), returns 0 if not a valid anchor
 */
MINKBASE_EXPORT_F unsigned long anchorToTimestamp(const char* anchor);

MINKBASE_EXPORT_F char* stringdup(const char* s, size_t len = STRINGDUP_NOLEN) ;
MINKBASE_EXPORT_F WCHAR* wstrdup(const WCHAR* s, size_t len = STRINGDUP_NOLEN);

MINKBASE_EXPORT_F char*  strtolower(const char *s);
MINKBASE_EXPORT_F WCHAR* wcstolower(const WCHAR *s);

MINKBASE_EXPORT_F char*  strtoupper(const char *s);
MINKBASE_EXPORT_F WCHAR*  wcstoupper(const WCHAR *s);

/**
 * find a substring from the end, with optional string lenght
 */
MINKBASE_EXPORT_F const char *brfind(const char *s1, const char *s2, size_t len=STRINGDUP_NOLEN) ;
...
*/
MINKBASE_EXPORT_F bool wcscmpIgnoreCase(const char*  p, const char*  q);

/*
* Converts a integer into a char*
*/
MINKBASE_EXPORT_F char*  itow(int i);

/*
* Converts a integer into a char*
*/
MINKBASE_EXPORT_F char*  ltow(long i);
...
*/
MINKBASE_EXPORT_F char*  MD5CredentialData(const char* userName, const char* password, const char* nonce);

/*
* Calculates the digest given the token and its lenght
*/
MINKBASE_EXPORT_F char* calculateMD5(const void* token, int len, char* wdigest);
 ...
 */
MINKBASE_EXPORT_F char *mkTempFileName(const char *name);
 ...
 */
MINKBASE_EXPORT_F bool saveFile(const char *filename, const char *buffer, size_t len,
              bool binary = false );
 ...
 */
MINKBASE_EXPORT_F size_t fgetsize(FILE *f);
 ...
 */
MINKBASE_EXPORT_F bool readFile(const char* name, char **message, size_t *len, bool binary = false );
 ...
 */
MINKBASE_EXPORT_F char** readDir(char* name, int *count, bool onlyCount = false);
...
*/
MINKBASE_EXPORT_F bool removeFileInDir(const char* dir, const char* filename = NULL);
 ...
 * time base as time().
 */
MINKBASE_EXPORT_F unsigned long getFileModTime(const char* name);

/**
* Return the cache directory where are stored all the file .dat
* used by the itemhandler cache implementation
*/
MINKBASE_EXPORT_F StringBuffer getCacheDirectory();

MINKBASE_EXPORT_F long int getLenEncoding(const char*  s, const char* encoding);
MINKBASE_EXPORT_F char *toMultibyte(const WCHAR *wc, const char *encoding = 0 );
MINKBASE_EXPORT_F WCHAR *toWideChar(const char *mb, const char *encoding = 0 );
...
//    return encodeBuf;
//}
MINKBASE_EXPORT_F const char *_wcc(const WCHAR *wc, const char *enc=0);
