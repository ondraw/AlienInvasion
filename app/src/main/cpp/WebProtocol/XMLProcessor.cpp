                          ...
                          unsigned int* endPos  )
{
    ...
    do {
        ...
        if (!p1) {
            // Tag not found
            //MinkTrace("XMLProcessor: tag %s not found", openTag);
            return 0;
        }
        ...
        switch( *p1 ){
            ...
            default:
                ...
                //MinkTrace("XMLProcessor: this is not tag %s", openTag);
                xmlptr = p1;
                p1 = 0;
        }
    }
...
void XMLProcessor::copyAttributeContent(
									  StringBuffer& res,
                                      const char* xml,
                                      const char* tag,
                                      ...
                                      unsigned int* pEnd)
{
    const char* pAttXML = NULL;
	unsigned int nStart = 0, nEnd = 0;
	*pEnd = nEnd;

	res.assign(NULL);

	if( (pAttXML = XMLProcessor::getElementAttributes (xml, tag, &nStart, &nEnd) ) != NULL)
	{		
		int nLen = strlen(attributename);
		pAttXML = strstr(pAttXML,attributename);
		if(pAttXML == NULL) return ;

        if(strncmp(pAttXML,attributename,nLen) == 0)
		{
			char* ns = strstr((char*)(pAttXML + nLen), "\"") + 1;
			char* ne = strstr(ns, "\"");
			int nResultLen = ne - ns;
			char* pResult = new char[nResultLen + 1];
			pResult[nResultLen] = 0;
			strncpy(pResult,ns,nResultLen);
			res = pResult;
			*pEnd  = (unsigned long)(ne + 1);
		}        
	}
}
...
const char* XMLProcessor::getElementAttributes(const char* xml,
    ...
    if (!p1) {
        //KANG
        //MinkTrace("XMLProcessor: tag %s not found", tag);
        goto finally;
    }
