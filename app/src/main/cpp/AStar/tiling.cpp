...
Tiling::Tiling(LineReader& reader)
{
    ...
    while (! done)
    {
        ...
        if (attribute == "type")
        {
            ...
            if (! in)
            {
                ...
                HLogE("[ERROR]Missing type value.");
                return;
            }
            if (typeString == "tile")
                type = TILE;
            ...
            else
            {
                ...
                HLogE("[ERROR]Invalid type value.");
                return;
            }
            typeFound = true;
        }
    ...
    if (columns == -1 || rows == -1)
    {
        ...
        HLogE("[ERROR]Map without valid width / height.");
        return;
    }
    if (! typeFound)
    {
        //throw reader.createError("Map without type.");
        HLogE("[ERROR]Map without type.");
        return;
    }
    init(type, rows, columns);
...
void Tiling::setObstacle(int nIndex,bool v)
{
    TilingNodeInfo& nodeInfo = m_graph.getNodeInfo(nIndex);
    nodeInfo.setObstacle(v);
}

bool Tiling::IsObstacle(int nIndex)
{
    TilingNodeInfo& nodeInfo = m_graph.getNodeInfo(nIndex);
    return nodeInfo.isObstacle();
}
