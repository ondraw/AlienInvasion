...
/** @file statistics.cpp
    ...
    $Id: statistics.cpp,v 1.6 2003/06/10 22:28:50 emarkus Exp $
...
void StatisticsCollection::add(const StatisticsCollection& collection)
{
    if (m_map.size() != collection.m_map.size())
    {
        ...
        HLogE("[ERROR]Trying to add incompatible statistics collections");
    }
...
{
    map<string,Statistics>::const_iterator p = m_map.find(name);
    if (p == m_map.end())
    {
        ostringstream o;
        o << "1Unknown statistics name " << name << '.';
        //throw Error(o.str());
        HLogE("[ERROR] %s",o.str().c_str());
    }
    return p->second;
}
...
Statistics& StatisticsCollection::get(const string& name)
{
    ...
    {
        ostringstream o;
        o << "2Unknown statistics name " << name << '.';
