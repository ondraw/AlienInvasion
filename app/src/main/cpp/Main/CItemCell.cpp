...
CItemCell::CItemCell(CTextureMan *pTextureMan) : CCellCtl(pTextureMan)
{
    mpBombInfo = 0;
    mnBeforeCnt = 0;
}
...
int CItemCell::Initialize(USERINFO* pBombInfo,int nWidth,int nHeight,vector<string>& lstImage,float fsu,float fsv,float feu,float fev)
{
...
int CItemCell::RenderBegin(int nTime)
{
    ...
    {
        mnBeforeCnt = mpBombInfo->nCnt;
        ((CNumberCtl*)mlstChild[1])->SetNumber(mnBeforeCnt);
    }
